...
Operuje na skillach GLOBALNYCH (`~/.claude/skills/`); skille projektowe — v2.
...
- **Zakres = lokalizacja:** tykasz WYŁĄCZNIE `~/.claude/skills/*`. Skille pluginów
  (`~/.claude/plugins/...`) są poza tym drzewem — NIE ruszasz.
...
1. **Skanuj** `~/.claude/skills/*/FRICTION.md`; zbierz pliki z ≥1 ważnym wpisem (komenda niżej).
...
for(const d of fs.readdirSync(root)){
  const f=path.join(root,d,"FRICTION.md");
  if(!fs.existsSync(f)) continue;
  const t=fs.readFileSync(f,"utf8");
  const n=(t.match(/^\s*-\s*\*\*expected:\*\*/mg)||[]).length;
  if(n===0) continue;
  console.log(d, n+" wpis(y)");
}'
```

## Verification

- Lista zawiera wyłącznie skille z `~/.claude/skills/*` mające ważny wpis (`expected`).
